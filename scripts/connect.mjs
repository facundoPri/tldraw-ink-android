...
if (!page) throw Error('WebView is unavailable')
let share = ''; for await (const chunk of process.stdin) share += chunk
if (!share.trim()) throw Error('Provide the invitation through stdin')
await page.locator('#share').fill(share.trim())
await page.getByRole('button', {name:'Open board'}).click()
try {await page.waitForFunction(() => !!window.editor, {timeout:20000})} catch {console.log((await page.locator('body').innerText()).slice(0,2000)); throw Error('Editor did not mount')}
