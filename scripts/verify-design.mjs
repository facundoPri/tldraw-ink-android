...
const context = await browser.newContext({viewport:{width:1204,height:752},deviceScaleFactor:1})
...
try {
 ...
 await page.getByLabel('Ver participantes').click()
 assert.match(await page.locator('.participants').innerText(), /Vos · tablet/)
 await page.getByLabel('Ver participantes').click()
 ...
 await page.getByLabel('Agregar un enlace Share').waitFor()
 ...
 assert(await page.getByRole('button',{name:'Abrir board'}).isDisabled())
 await page.getByLabel('Agregar un enlace Share').fill('invalid')
 await page.getByRole('button',{name:'Abrir board'}).click()
 assert((await page.locator('[role=alert]').innerText()).length > 0)
 assert.deepEqual(errors, [])
 await writeFile('artifacts/design-results.json',JSON.stringify({checks:['canvas renders','participant menu opens','dark theme controls','narrow layout and accessible Boards button','Boards navigation','empty input disabled','invalid link error'],dark,errors},null,2))
