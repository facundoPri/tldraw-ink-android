...
assert(doc, 'El board compartido original debe estar abierto')
...
try {
 ...
 await poll(async()=>!!await exec(`return !!editor.getShape(${JSON.stringify(a)})`),'Android crea → Mac recibe')
 await android(`window.editor.updateShape({id:${JSON.stringify(a)},type:'geo',x:1540,y:930})`)
 await poll(async()=>await exec(`return editor.getShape(${JSON.stringify(a)})?.x === 1540`),'Android mueve → Mac recibe')
 await android(`window.editor.deleteShapes([${JSON.stringify(a)}])`)
 await poll(async()=>await exec(`return !editor.getShape(${JSON.stringify(a)})`),'Android borra → Mac recibe')
 await exec(`editor.createShape({id:${JSON.stringify(b)},type:'geo',x:1500,y:900,props:{w:150,h:110,color:'green'}}); return true`)
 await poll(()=>android(`!!window.editor.getShape(${JSON.stringify(b)})`),'Mac crea → Android recibe')
 await exec(`editor.updateShape({id:${JSON.stringify(b)},type:'geo',x:1640,y:1040}); return true`)
 await poll(()=>android(`window.editor.getShape(${JSON.stringify(b)})?.x===1640`),'Mac mueve → Android recibe')
 await exec(`editor.deleteShapes([${JSON.stringify(b)}]); return true`)
 await poll(()=>android(`!window.editor.getShape(${JSON.stringify(b)})`),'Mac borra → Android recibe')
} finally {
